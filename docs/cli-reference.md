...
用于将 `wecom-cli` 作为标准 `stdio MCP server` 暴露给阿里百炼等平台。
...
说明：
- 仅支持 `stdio` 传输。
- 对外暴露的工具名格式为 `<category>.<method>`，例如 `contact.get_userlist`。
- 若未找到本地缓存的 MCP 配置，服务会自动刷新配置。
...
| `WECOM_CLI_BOT_ID` | 非交互环境下的企业微信机器人 Bot ID |
| `WECOM_CLI_BOT_SECRET` | 非交互环境下的企业微信机器人 Secret |
