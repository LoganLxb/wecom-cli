...
use crate::{constants, crypto, fs_util, paths};
...
pub fn get_bot_info() -> Option<Bot> {
    if let Some(bot) = get_bot_info_from_env() {
        return Some(bot);
    }
...
fn get_bot_info_from_env() -> Option<Bot> {
    let bot_id = std::env::var(constants::env::BOT_ID).ok();
    let bot_secret = std::env::var(constants::env::BOT_SECRET).ok();

    match (bot_id, bot_secret) {
        (Some(id), Some(secret)) if !id.trim().is_empty() && !secret.trim().is_empty() => {
            Some(Bot::new(id, secret))
        }
        (Some(_), None) | (None, Some(_)) => {
            tracing::warn!(
                "检测到不完整的环境变量凭证，已忽略。请同时设置 {} 和 {}",
                constants::env::BOT_ID,
                constants::env::BOT_SECRET
            );
            None
        }
        _ => None,
